...
if EMBEDDINGS_MODEL_SETTING == "VOYAGE":
    voyage_key = os.getenv("VOYAGE_API_KEY")
    print("🤖 Mode: Connecting to Cloud Voyage Index (1024 Dim Space)...")
    embeddings = VoyageAIEmbeddings(voyage_api_key=voyage_key,
    model="voyage-3")
    INDEX_NAME = "fde-sop-index-voyage"
 
else:
    local_model_target = os.getenv("Local_Embedding_Model", "BAAI/bge-m3").strip()
    
    print("🤖 Mode: Connecting to Local BGE-M3 Index (1024 Dim Space)...")
    from langchain_huggingface import HuggingFaceEmbeddings
