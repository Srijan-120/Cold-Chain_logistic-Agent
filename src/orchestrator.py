...
if AGENT_LLM_SETTING == "OPENAI":
    ...
    print("⚡ Brain Mode: Utilizing Ultra-Fast Groq Cloud (llama-3.1-8b-instant)...")
    ...
    llm = ChatGroq(
        model="qwen/qwen3.8-27b",  # Or "llama-3.1-8b-instant"
        temperature=0,
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if is_first_turn:
            input_messages = [system_prompt, ("user", user_input)]
            is_first_turn = False
        else:
            input_messages = [("user", user_input)]
            
        events = fde_agent.stream({"messages": input_messages}, config=thread_config, stream_mode="updates")
        for event in events:
